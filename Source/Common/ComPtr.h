...
public:
  ...
  VOID Detach(__in VARIANT *lpTargetVar)
    {
    memcpy(lpTargetVar, &sVt, sizeof(VARIANT));
...
template <class T>
class TNktComSafeArrayAccess
{
public:
  TNktComSafeArrayAccess()
    {
    lpArray = NULL;
    lpData = NULL;
    return;
  ...
  TNktComSafeArrayAccess(__in SAFEARRAY *lpArray)
    {
    Setup(lpArray);
    return;
    };
  ...
  HRESULT Setup(__in SAFEARRAY *_lpArray)
  {
    HRESULT hRes = S_OK;
    ...
    if (_lpArray != NULL)
    {
      hRes = ::SafeArrayAccessData(_lpArray, (void HUGEP**)&lpData);
      if (SUCCEEDED(hRes))
        lpArray = _lpArray;
      else
        lpData = NULL;
