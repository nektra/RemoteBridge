...
public:
  ...
  SIZE_T BinarySearch(__in TType *lpKey, __in _Comparator lpSearchFunc, __in_opt LPVOID lpContext=NULL)
    ...
    return (lpRes != NULL) ? (SIZE_T)(lpRes-lpItems) : (SIZE_T)(-1);
    };
    ...
    if (_nSize != nSize)
    {
      if (_nSize>nSize || bForceRealloc!=FALSE)
      {
        ...
        if (_nSize > 0)
        {
          ...
          lpNewItems = (TType*)NKT_MALLOC(_nSize*sizeof(TType));
          if (lpNewItems == NULL)
            return FALSE;
        }
        else
        ...
        if (_nSize > 0)
        {
          ...
          memcpy(lpNewItems, lpItems, i*sizeof(TType));
...
class TNktArrayListWithFree : public TNktArrayList<TType,nGranularity, TNktArrayListItemRemove_Free<TType>>
{
};
...
template <typename TType, SIZE_T nGranularity=32>
class TNktArrayListWithRelease : public TNktArrayList<TType,nGranularity, TNktArrayListItemRemove_Release<TType>>
{
};
...
template <typename TType, SIZE_T nGranularity=32>
class TNktArrayListWithDelete : public TNktArrayList<TType,nGranularity, TNktArrayListItemRemove_Delete<TType>>
{
};
...
template <typename TType, SIZE_T nGranularity=32, class ItemRemoveClass=TNktArrayListItemRemove<TType>>
class TNktArrayList4Structs : protected ItemRemoveClass
{
public:
  TNktArrayList4Structs() : ItemRemoveClass()
    {
    lpItems = NULL;
    nCount = nSize = 0;
    return;
  ...
  virtual BOOL SetElementAt(__in TType *lpElem, __in SIZE_T nIndex)
    ...
    if (nIndex >= nCount)
      return FALSE;
    memcpy(&lpItems[nIndex], lpElem, sizeof(TType));
    return TRUE;
    };
    ...
    if (_nSize != nSize)
    {
      if (_nSize > nSize || bForceRealloc != FALSE)
      {
        ...
        if (_nSize > 0)
        {
          ...
          memcpy(lpNewItems, lpItems, k*sizeof(TType));
          //initialize the rest to null
          if (_nSize > nCount)
            memset(&lpNewItems[nCount], 0, (_nSize-nCount)*sizeof(TType));
        }
