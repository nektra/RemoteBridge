                               ...
                               __in va_list argptr)
{
  ...
  if (sDispParams.cArgs != 0)
  {
    sDispParams.rgvarg = (VARIANTARG*)NKT_MALLOC((size_t)sDispParams.cArgs * sizeof(VARIANTARG));
    if (sDispParams.rgvarg == NULL)
      return E_OUTOFMEMORY;
    memset(sDispParams.rgvarg, 0, (size_t)sDispParams.cArgs * sizeof(VARIANTARG));
    // get ready to walk vararg list
    lpCurrParam = (LPBYTE)lpParameters;
    lpArg = sDispParams.rgvarg + (sDispParams.cArgs - 1); // params go in opposite order
    while (*lpCurrParam != 0)
    {
      _ASSERT(lpArg >= sDispParams.rgvarg);
  ...
  lpVarResult = NULL;
  memset(&sVarResult, 0, sizeof(sVarResult));
  if (vtRet != VT_EMPTY)
    lpVarResult = &sVarResult;
  nArgErr = (UINT)-1;  // initialize to invalid arg
  memset(&sExcepInfo, 0, sizeof(sExcepInfo));
  // make the call
...
static LPWSTR Ansi2Wide(__in_nz_opt LPCSTR szSrcA, __in SIZE_T nSrcLen)
{
  ...
  szDestW = (LPWSTR)NKT_MALLOC((nDestLen+1)*sizeof(WCHAR));
  if (szDestW == NULL)
    return NULL;
