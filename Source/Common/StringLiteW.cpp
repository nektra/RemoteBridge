...
BOOL CNktStringW::EnsureBuffer(__in SIZE_T nChars)
{
  ...
  if (nChars >= nSize)
  {
    ...
    szNewStrW = (LPWSTR)NKT_MALLOC(nChars*sizeof(WCHAR));
    if (szNewStrW == NULL)
      return FALSE;
...
LPWSTR CNktStringW::Duplicate(__in LPWSTR szStrW, __in SIZE_T nLen)
{
  ...
  if (nLen == (SIZE_T)-1)
    nLen = wcslen(szStrW);
  szStrCopyW = (LPWSTR)NKT_MALLOC((nLen+1) * sizeof(WCHAR));
  if (szStrCopyW == NULL)
    return NULL;
  memcpy(szStrCopyW, szStrW, nLen*sizeof(WCHAR));
  szStrCopyW[nLen] = 0;
  return szStrCopyW;
}

LPSTR CNktStringW::Wide2Ansi(__in_nz_opt LPCWSTR szSrcW, __in SIZE_T nSrcLen)
{
  SIZE_T nDestLen;
  LPSTR szDestA;
